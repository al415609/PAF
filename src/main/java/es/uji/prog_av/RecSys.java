...
public class RecSys{
    ...
    public void run(Table testData, List<String> testItemNames){
        ...
        for(int i = 0; i < testData.n_filas(); i++){
            estimates.add((Integer) algorithm.estimate(testData.getRowAt(i).getData()));
            this.testItemNames.add(testItemNames.get(i));
        }
    }
    ...
    private void selecItems(int idxLikedItem, int labelLikedItem, int numRec){
        int c = 0;
        for(int i = 0; i < testData.n_filas(); i++){
            if(estimates.get(i) == labelLikedItem && !testItemNames.get(i).equals(testItemNames.get(idxLikedItem))){
                String nombreSeleccionado = testItemNames.get(i);
                if(!selectedItems.contains(nombreSeleccionado))
